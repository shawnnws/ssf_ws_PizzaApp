package sg.com.nus.iss.workshop_pizzaapp.service;
...
import sg.com.nus.iss.workshop_pizzaapp.repository.PizzaRepository;
...
public class PizzaService {
    ...
    // Uppercase variable names are for variables that are contants, values that do not change throughout.

    // Defining pizza types
    public static final String[] PIZZA_NAMES = {
        "bella", "margherita", "marinara", "spianatacalabrese", "trioformaggio"
    };

    // Defining pizza sizes
    ...
    @Value("${workshop_pizzaapp.pizza.api.url}")
    private String restPizzaUrl;

    // Assign the fixed values into Hashsets
    public PizzaService(){
    ...
    // Is this necessary since the way the order form works is such that the customer selects from the list of pizzas on the webpage?
    // This is a good to know function to validate the order NOT on the object level
    public List<ObjectError> validatePizzaOrder(Pizza p){
        List<ObjectError> errors = new LinkedList<>();
        FieldError error;

        // There are 2 instances of "pizza" because we are trying to validate an attribute under the Pizza object, which is the type of Pizza
    ...
    // This getOrderDetails is done here instead of repository because we are retrieving from Controller instead of directly from database.
